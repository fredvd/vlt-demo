...
2. Go to your repository at [fredvd/vlt-demo](https://github.com/fredvd/vlt-demo).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `vlt-demo.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | vlt-demo.example.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/fredvd/vlt-demo/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/fredvd/vlt-demo/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/fredvd/vlt-demo-backend`) and Frontend (`ghcr.io/fredvd/vlt-demo-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of vlt-demo.example.com](https://github.com/fredvd/vlt-demo/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/vlt-demo.example.com.yml` stack.
