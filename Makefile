...
PROJECT_NAME=vlt-demo
STACK_NAME=vlt-demo-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://vlt-demo.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t fredvd/vlt-demo-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t fredvd/vlt-demo-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name vlt-demo-backend-acceptance -d fredvd/vlt-demo-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name vlt-demo-frontend-acceptance --link vlt-demo-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d fredvd/vlt-demo-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop vlt-demo-frontend-acceptance
	@docker stop vlt-demo-backend-acceptance
